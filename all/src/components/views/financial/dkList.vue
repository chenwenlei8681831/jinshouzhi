                ...
                <div class="views_100ch_1">
                    ...
                    <div class="flex row w_100 header_item h_60px b_s pad_r_30 pad_l_30">
                        ...
                        <div class="w_25 flex row b_s h_100 mar_l_20">
                            <span class="font12">{{seleteIndex==0?'申请时间':(seleteIndex==1?'结清时间':'取消时间')}}：</span>
                            <el-date-picker
                                    class="flex_1"
                                    v-model="timer"
                    ...
                    <div class="w_100 header_item h_60px b_s font14">
                        <!--还款-->
                        <span v-show="seleteIndex==0&&permissions['finance.daikouback']==1" @click="huankuan_choose()" class="search_btn 3_btn b_s pad_r_20 pointer all_line f_l do_btn" style="font-size:14px">批量还款</span>
                    ...
                    <div class="list">
                        <ol class="list_header" :class="seleteIndex==0?'current01':(seleteIndex==1?'current02':'current03')">
                            ...
                            <li class="h_50px pad_l_20 b_s f_l">{{seleteIndex==0?'申请时间':(seleteIndex==1?'结清时间':'取消时间')}}</li>
                            <li class="h_50px pad_l_20 b_s f_l">{{seleteIndex==0?'代扣金额':(seleteIndex==1?'已结清金额':'代扣金额')}}</li>
                            <li class="h_50px pad_l_20 b_s f_l" v-if="seleteIndex==0||seleteIndex==2">已还金额</li>
                            <li class="h_50px pad_l_20 b_s f_l" v-show="seleteIndex==0">还款金额</li>
                            <li class="h_50px pad_l_20 b_s f_l">状态</li>
                            <li style="width:11%;" v-show="seleteIndex==0" class="h_50px pad_l_20 b_s f_l caozuo">操作</li>
                            <div class="clearfix"></div>
                        </ol>
                        <ul class="list_main">
                            <li class="" v-for="(item,index) in list" :key="index">
                                <ol :class="seleteIndex==0?(index%2==1?'list_bg current01':'current01'):(seleteIndex==1?(index%2==1?'list_bg current02':'current02'):(index%2==1?'list_bg current03':'current03'))">
                                    ...
                                    <li class="h_50px pad_l_20 b_s f_l" :title="item.center_name?item.center_name:'--'">{{item.center_name?item.center_name:'--'}}</li>
                                    <li class="h_50px pad_l_20 b_s f_l" :title="item.time?item.time:'--'">{{item.time?item.time:'--'}}</li>
                                    <li class="h_50px pad_l_20 b_s f_l" :title="item.money?item.money:0+'元'">{{item.money?item.money:0}}元</li>
                                    <li class="h_50px pad_l_20 b_s f_l" v-if="seleteIndex==0||seleteIndex==2" :title="item.back_money?item.back_money:0+'元'">{{item.back_money?item.back_money:0}}元</li>
                                    <li class="h_50px pad_l_20 b_s f_l"  v-show="seleteIndex==0">
                                          <input class="current" placeholder="请输入" oninput="value=value.replace(/[^\d\.]/g,'')" v-model="item.xianzai_money">
                                    </li>
                                    <li class="h_50px pad_l_20 b_s f_l" :class="seleteIndex==0?'list_co02':(seleteIndex==1?'list_co03':'list_co01')">{{seleteIndex==0?'未结清':(seleteIndex==1?'已结清':'已取消')}}</li>
                                    <li  style="width:11%;" v-show="seleteIndex==0" class="f_l pad_l_20 b_s flex row caozuo" >
                                        <span v-show="permissions['finance.daikouback']==1" class="edit_button flex_1" @click="view(1,item.id,index)">还款</span>
                                        <span v-show="permissions['finance.daikoucancel']==1" class="edit_button flex_1 list_co02" @click="view(2,item.id,index)">取消</span>
                                    </li>
                                    <div class="clearfix"></div>
                                </ol>
                            </li>
...
export default {
  ...
  data(){
      return {
            ...
            allFlag:false,
            seleteArr:[
                {name:'未结清',number:'0',type:0},
                {name:'已结清',number:'0',type:1},
                {name:'已取消',number:'0',type:2},
            ],
            list:[],
            huankuan_status:0,
            companyArr:[],
  ...
  methods:{
        ...
        getList(){
                ...
                vue_Post(THIS,'finance/daikouList',data,callback,0,0,0,1);
                function callback(msg){
                    THIS.allFlag = false;
                    THIS.list = msg.list;
                    THIS.total = msg.total;
                    THIS.seleteArr[0].number = msg.wj_total?msg.wj_total:0;
                    THIS.seleteArr[1].number = msg.yj_total?msg.yj_total:0;
                    THIS.seleteArr[2].number = msg.cancel_total?msg.cancel_total:0;
                    for(let i=0;i<THIS.list.length;i++){
                        THIS.list[i].time = THIS.seleteIndex==0?THIS.list[i].createat:(THIS.seleteIndex==1?THIS.list[i].jieqingat:THIS.list[i].jieqingat);
                        THIS.list[i].money = THIS.seleteIndex==0?THIS.list[i].money:THIS.list[i].back_money;
        ...
        view(type,id,index){
            THIS.$confirm('确定'+(type==1?'还款':'取消')+'吗?', '提示', {
        ...
        cancelFunc(id){
            var data = { id:id };
            vue_Post(THIS,'finance/daikouCancel',data,callback,1,1,1,1);
            function callback(msg){
                THIS.getList();
                THIS.$message({
                    type: 'success',
        ...
        huankuanFunc(value){
            ...
            vue_Post(THIS,'finance/daikouBack',data,callback,1,1,1,1);
            function callback(msg){
                THIS.getList();
                THIS.$message({
                    type: 'success',
        ...
        downLode(ids){
                ...
                var bb = window.localStorage.host+'finance/daikouExport?token='
   ...
   mounted(){
        ...
        THIS.store.commit('setPageText','当前位置：财务 / 代扣管理');
        THIS.search_company('');
        THIS.getList();
